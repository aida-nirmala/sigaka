$(document).ready(function () {
	...
	$('.tenagahonorer-lihat').click(function (e) {
		e.preventDefault();
		var id = $(this).val();
		var getUrl = root + 'tenagahonorer/lihat/' + id;
		$.ajax({
			url : getUrl,
			type : 'ajax',
			dataType : 'json',
			success: function (response) {
				if (response != null){
					$('#lihat_nama').val(response.tenagahonorer_nama);
					$('#lihat_tempat').val(response.tenagahonorer_tempat_lahir);
					$('#lihat_tl').val(response.tenagahonorer_tanggal_lahir);
					$('#lihat_alamat').val(response.tenagahonorer_alamat);
					$('#lihat_nohp').val(response.tenagahonorer_nomor_hp);
					$('#lihat_norek').val(response.tenagahonorer_no_rekening);
					$('#lihat_tg').val(response.tenagahonorer_tanggal_gabung);
					$('#lihat_jabatan_tenagahonorer').val(response.jabatan_nama);
	...
	$('.tenagahonorer-edit').click(function (e) {
		e.preventDefault();
		var id = $(this).val();
		var getUrl = root + 'tenagahonorer/lihat/' + id;
		$.ajax({
			...
			success: function (response) {
				if (response != null){
					$('#tenagahonorer_id').val(response.tenagahonorer_id);
					$('#edit_nama').val(response.tenagahonorer_nama);
					$('#edit_tempat').val(response.tenagahonorer_tempat_lahir);
					$('#edit_tl').val(response.tenagahonorer_tanggal_lahir);
					$('#edit_alamat').val(response.tenagahonorer_alamat);
					$('#edit_nohp').val(response.tenagahonorer_nomor_hp);
					$('#edit_norek').val(response.tenagahonorer_no_rekening);
					$('#edit_tg').val(response.tenagahonorer_tanggal_gabung);
					$('#edit_jabatan_tenagahonorer').val(response.gaji_jabatan);
	...
	$('.tenagahonorer-hapus').click(function () {
		var id = $(this).val();
		var html = '' +
			'<a href="'+root+'tenagahonorer/hapus/'+id+'" class="btn btn-danger btn-bg-gradient-x-red-pink">Hapus</a>';
		$('#hapustenagahonorer').html(html);
	});
	...
	// ------------------------------------------------------------------------------------------
	var options = {
		url : root + 'tenagahonorer/ajaxIndex',
		getValue: 'tenagahonorer_nama',
		adjustWidth : false,
		list: {
			sort: {
				enabled: true
			...
			onSelectItemEvent: function() {
				var value = $("#nama_tenagahonorer").getSelectedItemData().tenagahonorer_id;

				$("#id_tenagahonorer").val(value).trigger("change");
			}
		}
	};

	$('#nama_tenagahonorer').easyAutocomplete(options);
	...
	$('.gaji-slip').click(function (e) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response != null){
					$('.slip-nama').html(response.tenagahonorer_nama);
					$('#slip-jabatan').html(response.jabatan_nama);
					$('#slip-nohp').html(response.tenagahonorer_nomor_hp);
				...
				if (response != null){
					...
					for (var i = 0; i < response.length; i++){
						html += '' +
							'<tr>' +
							'<td>'+no+'</td>' +
							'<td>'+response[i].tenagahonorer_nama+'</td>' +
							...
							'<td style="text-align: right"> Rp. '+formatRupiah((parseInt(response[i].gaji_total) - parseInt(response[i].gaji_bayar_pinjaman)).toString())+'</td>' +
							'</tr>';
						total = total + (parseInt(response[i].gaji_total)) - parseInt(response[i].gaji_bayar_pinjaman);
						kotor = kotor + (parseInt(response[i].gaji_total));
					...
					html += '' +
						...
						'<p>Purwokerto, '+date_indo(d.getFullYear()+'-'+(d.getMonth()+1)+'-'+d.getDate())+'</p>' +
						'<p>Admin</p>' +
						'<br>' +
						'<br>' +
						'<br>' +
						'<p><b><u>Wahyu Aji</u></b></p>' +
						'</div>' +
						'</div>';
					$('#laporan').html(html);
				}
